...
## Stale DNS ghosts became VM static IPs

Terraform derives each VM's static IP by resolving its FQDN (`data.external.dns_lookup` → `scripts/dns-lookup.sh`) and feeds the answer into cloud-init's netplan. During the mushroom re-provision, the CI container's resolver was still serving stale DHCP-ghost registrations left behind by the failed first runs (e.g. `k8s-mushroom-worker-01` → `192.168.0.157`), and `dns-lookup.sh`'s fallback happily returned the out-of-pool answer. Two VMs therefore received static netplans on DHCP-range addresses (`.157`/`.143`) that disagreed with their in-pool DNS overrides (`.42`/`.43`) — the readiness gate passed (hostinfo matched the netplan), but the machines were on addresses outside the IaC pool and unreachable by name. **Solution:** the IaC pool is now authoritative in `dns-lookup.sh` — an out-of-pool answer is never returned. If the resolver yields no in-pool answer, the script queries pfSense's host-override API directly (the record `dns_alloc` just created; immune to resolver caches and DHCP-lease ghosts) and accepts the IP only when it is in-pool. The Docker host's stale resolver cache is covered by the existing DHCP-restart verification step, which retries resolution and auto-deletes surviving ghosts.
