...
### Added
- **Standby node pools** — per-plane `standby` integer on clusters (default `0`). Terraform provisions `nodes + standby` uniform VMs; Ansible + the Proxmox API enforce the partition: standby VMs are pre-built with the k8s base toolchain (shared `tasks/k8s-base.yaml` used by both `k8s-cluster.yaml` Play 1 and the new `k8s-standby-build.yaml`) then parked (clean shutdown + `standby` tag) by `scripts/reconcile-standby-nodes.sh`. Expanding the cluster (increase `nodes`, decrease `standby` — total flat) boots and joins the promoted ghost without rebuilding, ~3x faster than a cold build. Standby VMs are deliberately `iac`-invisible: Terraform stamps no `iac` tag on them and `extract_drifted_vms` filters them out. New CI steps: Wake Active Nodes (pre config-mgmt), Build and Park Standby Nodes (post config-mgmt), plus per-stage run timings via `scripts/record-timing.sh`/`report-timings.sh`
